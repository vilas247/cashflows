(function(root, factory) {
	...
	X247OrderDetails.cols_data = [
								{val:"invoice_id",name:"transaction ID"},
	...
	X247OrderDetails.main_data = function(main_url,table_id){
		...
		if(X247OrderDetails.cols_data != ""){
			var table_columns = '<th class="no-sort" ><input class="form-check-input" type="checkbox" id="ckbCheckAll" /></th>';
	...
	X247OrderDetails.change_limit_data = function(limit){
		...
		$('#orderdetails_dashboard').DataTable({
			...
			"serverSide": true,
			"ajax": {
				"url": app_base_url+"home/orderdetailsprocessing",
				"type": "POST",
				"data":jsonData,
				dataFilter: function(data){
					var json = $.parseJSON( data );
